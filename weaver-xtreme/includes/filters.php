...
function weaverx_smartmenu( $menu_id ) {
...
function weaverx_enqueue_gutenberg_block_editor_assets() {
	...
	// enqueue style file
	wp_enqueue_style( 'weaverx_gutenberg_fonts', WEAVERX_GOOGLE_FONTS_URL );    // load the Google Fonts the theme uses so they are available to the editor
...
function weaverx_replace_widget_area_filter( $area_name ) {
	...
	if ( $replace ) {       // see if the replacement widget area actually exists...
		if ( ! is_active_sidebar( $replace ) ) {
			?>
			<h3><?php echo esc_html__( 'Notice: Widget Area Not Found:', 'weaver-xtreme' ); ?> <em><?php echo $replace; ?> &larr; <?php echo $area_name; ?></em></h3>
