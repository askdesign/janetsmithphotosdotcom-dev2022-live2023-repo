...
if ( ! function_exists( 'weaverx_enqueue_styles_action' ) ) {
	...
	function weaverx_enqueue_styles_action() {
		...
		// note: theme settings override Child CSS if there

		if ( ! weaverx_use_inline_css_v5( weaverx_get_css_filename() ) ) { // no inline CSS - load generated style file
			$vers = weaverx_getopt( 'style_version' );
			if ( ! $vers ) {
				$vers = '1';
			} else {
			...
			wp_register_style( 'weaverxt-style-sheet', weaverx_get_css_url(), array( 'weaverx-style-sheet' ), $vers );
			wp_enqueue_style( 'weaverxt-style-sheet' );
		}
	}
}
...
do_action( 'weaverx_load_customizer' );    // load the customizer based option interface.
