...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'WeaverX_Save_WX_Settings' ) ) :
	...
	class WeaverX_Save_WX_Settings extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<input type="button" class="button-primary" name="wvrx_save_all" value="<?php echo esc_attr__( 'Download ALL Weaver Xtreme Settings', 'weaver-xtreme' ); ?>"/>
			<br/><br/>
			<hr/>
			...
			<input type="button" class="button-primary" name="wvrx_save" value="<?php echo esc_attr__( 'Download THEME RELATED Settings', 'weaver-xtreme' ); ?>"/>
			<br/><br/>
			<hr/>

			<?php
			echo '<h3>' . esc_html__( 'Save Current Settings to WP Database', 'weaver-xtreme' ) . '</h3>';
			echo wp_kses_post( __( 'Backup all current theme settings using the WordPress database.', 'weaver-xtreme' ) ); ?><br/><br/>
...
The previous Save buttons do <em>not</em> include advanced <em>Weaver Xtreme Plus</em> options like shortcodes or SmartMenu settings.', 'weaver-xtreme' )
				...
				<input type="button" class="button-primary" name="wvrx_save_xplus" value="<?php echo esc_attr__( 'Download ALL Settings, including Plus', 'weaver-xtreme' ); ?>"/>
				<?php
				echo "<br /><br /><h3>";
				echo esc_html__( 'Download Settings to Site Host Filesystem', 'weaver-xtreme' );
				echo '</h3><p>';
	...
	class WeaverX_Restore_WX_Settings extends WP_Customize_Control {
		...
		static private function _restore( $wp_customize ) {
			...
			if ( $ok && $ext_check != 'wxt' && $ext_check != 'wxb' && $ext_check != 'wxall' && $ext_check != 'json' ) {
				self::$wvrx_error = esc_html__( 'Theme files must have .wxt, .wxb, .wxall, or .json extension.', 'weaver-xtreme' );
		...
		public function render_content() {
			...
			switch ( $level ) {
				...
				case 'what':
					...
					echo esc_html__( 'What', 'weaver-xtreme' );
					break;

				default:
					echo '<span style="background-color:gold;color:black;padding:3px;">';
					echo esc_html__( 'Where', 'weaver-xtreme' );
					break;
			}
			echo '</span>';
			?>
			...
			<?php echo '<h3>' . esc_html__( 'Select Customizer Interface Level', 'weaver-xtreme' ) . '</h3><p>'; // customizer only?>
			<?php echo esc_html__( 'Please click one of the buttons to set the Options Interface Level.', 'weaver-xtreme' ); ?>
			</p><p>

				<input style="background-color:silver;color:black;" type="button" class="button" name="wvrx_cust_interface_what" value="<?php echo esc_attr__( 'What', 'weaver-xtreme' ); ?>"/>
				&nbsp; Customizer Interface Menus organized by <strong>What</strong> is being set. What includes colors, style, typography, etc.
				<br/><br/>
				<input style="background-color:gold;color:black;" type="button" class="button" name="wvrx_cust_interface_where" value="<?php echo esc_attr__( 'Where', 'weaver-xtreme' ); ?>"/>
				&nbsp; Customizer Interface Menus organized by <strong>Where</strong> content is being set. Where includes area such as the Header, Wrappers, Content, and Footer.
			</p>
